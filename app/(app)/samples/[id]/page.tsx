...
export default async function EditOpportunityPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ blocked?: string }> }) {
  ...
  const boundUpdate = updateOpportunity.bind(null, id); const boundDelete = deleteOpportunity.bind(null, id);
  const confidence = CONFIDENCE[o.receipt_confidence ?? "unknown"];
  const images = [
    o.pr_package_image_url ? {url:o.pr_package_image_url,label:"PR package / campaign image"} : null,
    o.evidence_image_url ? {url:o.evidence_image_url,label:"Evidence image"} : null,
    o.product_image_url ? {url:o.product_image_url,label:"Official / product image"} : null,
  ].filter(Boolean) as Array<{url:string;label:string}>;

  return <div className="flex flex-col gap-6">
    <div><Link href="/samples" className="inline-flex items-center gap-1 text-sm text-muted-foreground hover:text-foreground"><ArrowLeft className="size-3.5" />Back to Samples</Link>
      <div className="mt-2 flex flex-wrap items-center gap-3"><h1 className="text-xl font-semibold tracking-tight">{o.brand} — {o.product}</h1><SafetyBadgePill badge={o.safety_badge} /><ScoreBadge score={o.opportunity_score} />{o.fulfillment_type === "loan_showroom_pull" ? <span className="rounded-full bg-orange-500/15 px-2.5 py-1 text-xs font-semibold text-orange-700 dark:text-orange-400">RETURN REQUIRED — Loan / Showroom Pull</span> : null}</div>
    </div>

    <Card>
      <CardHeader><CardTitle>What You Are Most Likely to Receive</CardTitle></CardHeader>
      <CardContent className="flex flex-col gap-5">
        ...
        <p className="text-xs text-muted-foreground">Product images show evidence, not a guarantee. Exact contents are only labeled “Exact item confirmed” when the campaign, brand, PR agency, creator platform, or email explicitly identifies them.</p>
      </CardContent>
    </Card>

    <Card>
      <CardHeader><CardTitle>Application / Request Activity</CardTitle></CardHeader>
      <CardContent>
        {history.length ? <div className="flex flex-col gap-3">{history.map((e) => <div key={e.id} className="grid gap-1 border-b border-border pb-3 last:border-0 last:pb-0 sm:grid-cols-[160px_1fr]"><div><Badge variant="outline">{e.event_type.replaceAll("_", " ")}</Badge><p className="mt-1 text-xs text-muted-foreground">{new Date(e.event_at).toLocaleString()}</p></div><div><p className="text-sm">{e.details || "Status event recorded"}</p><p className="mt-1 text-xs text-muted-foreground">Source: {e.source}</p></div></div>)}</div> : <p className="text-sm text-muted-foreground">No application events recorded yet.</p>}
        <p className="mt-4 text-xs text-muted-foreground">“Viewed” is only recorded when a supported source provides a real view/open signal. Gmail reply activity alone does not prove an email was opened.</p>
      </CardContent>
    </Card>
    ...
    <form action={boundDelete} className="flex justify-end"><Button type="submit" variant="destructive" size="sm">Delete Opportunity</Button></form>
  </div>;
}
